...
# Write the answer in a comment
# Q: Which state had the fewest total covid deaths August 30? How many deaths?
...
# Write the answer in a comment
# Q: What was the average number of cases per county in each state? Which state had the lowest average?
# A: Vermont with just under 115 cases per county
...
# A: New Jersey with nearly 759 deaths per county
...
# A: Because the District of Columbia is not considered a state, Delaware is the state with the smallest number of counties -- with three.
...
state_max_county_deaths <- covid_county %>%
  group_by(state) %>%
  summarise(average_deaths = max(deaths)) %>%
  arrange(desc(average_deaths))

state_max_county_deaths

# Q: Which state had the county with the highest number of deaths?
# A: New York had a county or city with 23,689 deaths
