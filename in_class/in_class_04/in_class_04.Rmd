...
# Write the answer in a comment
# Q: During the seven day heat wave, what was the lowest recorded heat index value in Stephanie's house?  What was the highest heat index value, and on what day did it occur?
# A: lowest 88.8 on July 16 and highest 115.7 on July 19
...
michael_daily_average <- michael_day_hourly_averages %>%
  group_by(date) %>%
  summarise(heat_index_averages=mean(mean_indoor_heat_index),             heat_index_max=max(mean_indoor_heat_index),
            heat_index_min=min(mean_indoor_heat_index), count=n())



michael_daily_average
...
* Q: Is there anything concerning the number of hourly readings on each day that could affect the results you're getting?
  A: The lower number of hourly readings from July 20 could affect that day's results.
